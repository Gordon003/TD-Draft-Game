package  Objects{
	...
	public class TeamCard extends MovieClip{
		...
		public function calculatePlayerChemistry(num){
			...
			teamCard = [MovieClip(root).Leader, MovieClip(root).Card1, MovieClip(root).Card2, MovieClip(root).Card3, MovieClip(root).Card4, MovieClip(root).Card5, MovieClip(root).Card6, MovieClip(root).Card7, MovieClip(root).Card8];
			...
			for (var J: int = 0; J < teamCard.length; J++){
				...
				if (teamMate != "None"){
					...
					for (var b: int = 0; b < enemyData.length; b++){
						if (teamMate.search(enemyData[b]) >= 0){
							if (teamMate.search("x2") >= 0){
								chemistry -= 2;
							}
							else{
								chemistry -= 1;
					...
					/*LEGEND
						If that teammate is a legend, the player get a +1 chemistry boost*/
